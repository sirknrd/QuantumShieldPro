...
def get_recommendation(df: pd.DataFrame) -> dict:
    ...
    if score >= 65:
        return {"label": "COMPRA FUERTE", "color": "#00D18F", "score": int(score), "confidence": 85, "reason": " • ".join(reasons)}
    elif score >= 30:
        return {"label": "COMPRA", "color": "#2F81F7", "score": int(score), "confidence": 72, "reason": " • ".join(reasons)}
    elif score <= -65:
        return {"label": "VENTA FUERTE", "color": "#FF4B4B", "score": int(score), "confidence": 85, "reason": " • ".join(reasons)}
    elif score <= -30:
        return {"label": "VENTA", "color": "#FFA657", "score": int(score), "confidence": 72, "reason": " • ".join(reasons)}
    else:
        return {"label": "NEUTRAL", "color": "#8B949E", "score": int(score), "confidence": 55, "reason": "Mercado en rango"}
...
def main():
    ...
    change = ((last_price / prev_price - 1) * 100) if prev_price > 0.01 else 0.0   # ← CORREGIDO
